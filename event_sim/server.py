...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
        ...
        if path.startswith("/snapshots/"):
            ...
            if (
                ...
                # (credential, then request body, then organization, then
                # snapshot ownership), so it is routed directly; every other
                # unknown /snapshots/... POST sub-path keeps the generic 404.
    ...
    def _evaluate_snapshot_decision(self, snapshot_id: str) -> None:
        ...
        events — the single-snapshot counterpart of the two-snapshot window
        decision (``POST /snapshots/compare``), reusing the exact request
        contract and window division of ``POST /decisions/evaluate``. The
        verdict order is fixed: the credential is authenticated first, then
        the request body is validated, then the requested organization is
        ...
        # The snapshot must already exist; a decision never implicitly creates
        # one, and a foreign snapshot is forbidden rather than not found.
        snapshot = self.server.snapshots.get(snapshot_id)  # type: ignore[attr-defined]
        if snapshot is None:
            self._snapshot_not_found()
        ...
        # filtered copy with the same start grid the main decision and the
        # snapshot window comparison use, so a snapshot compared left (and
        # right) against itself matches the comparison's decision item by
        # item. Nothing is written, so identical requests return
        # byte-identical JSON.
        occurred = snapshot.occurred_at_values(params["type"])

...
class ReservationInventory:
    ...
    def resource_balances_for_organization(
        ...
        Mirrors :meth:`Snapshot.resource_balances`: every resource the
        organization reserves maps to its ``capacity``, ``occupied`` (the
        total quantity it has reserved against it) and ``remaining``
        (``capacity - occupied``). The balances are computed inside the
        inventory lock from a consistent snapshot, so a read-only comparison
        ...
        with self._lock:
            ...
            for record in self._reservations.values():
                ...
                view["occupied"] += record["quantity"]
            for view in balances.values():
                view["remaining"] = view["capacity"] - view["occupied"]
        return balances
...
def _compare_resources(
    ...
    Each mapping is a single locked snapshot of one side's per-resource
    balances for one organization (branch inventory or captured snapshot),
    keyed by resourceId, so the rows and four groups derive from consistent
    reads and nothing is written. The union of the two sides' resource ids
    ...
    that side carries all three balances as zero. A row's ``equal`` marker is
    true exactly when the two sides' three balances all agree.
...
def compare_branch_resources(
    ...
    locked snapshot of one branch inventory's per-resource
    ``capacity``/``occupied``/``remaining`` balances for one organization.
    """
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _compare_branch_resources(self) -> None:
        ...
        # reservations contribute), then the comparison runs purely from
        # those copies; neither branch, the main service, nor any alert state
        # is written, so identical submissions return byte-identical JSON.

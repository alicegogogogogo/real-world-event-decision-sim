...
def reservation_rows(
    ...
    written. Each captured reservation maps to one row carrying the five
    reservation fields — ``organizationId``, ``reservationId``,
    ``resourceId``, ``quantity`` and ``capacity``; rows are sorted by
    resourceId, then reservationId, in Unicode code-point order, and an
    empty mapping yields an empty row array.
    """
    rows = [
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
        ...
        if path.startswith("/snapshots/"):
            ...
            if (
                ...
                # verdict order (credential, then query shape, then
                # organization, then snapshot), so it is routed directly too.
    ...
    def _list_snapshot_reservations(self, snapshot_id: str, query: str) -> None:
        ...
        single-snapshot counterpart of POST /snapshots/compare/reservations.
        The verdict order is fixed: the credential is authenticated first,
        then the query shape is validated, then the requested organization
        is compared with the credential's, and only then is the snapshot
        name resolved (missing snapshot, then foreign snapshot). Both
        ...
        # aligns on, so a snapshot compared with itself reports every listed
        # reservation in its same group with zero diffs. Nothing is written,

...
A read-only, resource-capacity comparison of two existing snapshots. The
other snapshot comparisons align events or reservations; this entry point
aligns the capacities and reservation balances the two snapshots captured at
their own creation times, by `resourceId`, and reports one balance row per
resource plus the deterministic difference summary. Nothing in either
snapshot, in any branch, or in the main service is read for mutation or
written — snapshots are immutable, alert state is untouched, and identical
submissions return byte-for-byte identical JSON. Both `read` and `write`
credentials may call it. Requires `Content-Type: application/json` and a
Bearer credential bound to the request's `organizationId`. The body must be
a JSON object containing exactly these fields:
...
Using the same snapshot name for `left` and `right` is legal; the two sides
are then the same capture and every row is equal.

A snapshot captures a resource's `capacity` (fixed by the first reservation
that names it) and its reservations; `occupied` is the total captured
quantity reserved against it and `remaining` is `capacity - occupied`. A
resource missing from one side is treated as not existing there, so that
side's `capacity`, `occupied`, and `remaining` all read as zero. Resources
are aligned by `resourceId`:

- `resources` — one row per resource in the union of the two sides, sorted
  by `resourceId` in Unicode code-point order. Each row is
  `{"resourceId", "left", "right", "equal"}`, where `left`/`right` each
  carry `capacity`, `occupied`, and `remaining`, and `equal` is true exactly
  when all three pairs agree (a resource absent on one side gets zeros, so
  an only-one-side resource is unequal).
- `leftOnly` — identifiers that appear only in the left snapshot.
- `rightOnly` — identifiers that appear only in the right snapshot.
...
- `diff` — identifiers present on both sides that disagree in at least one
  ...
  `fields` names the mismatched fields, drawn only from `capacity`,
  `occupied`, and `remaining`.

Identifiers and field names are sorted in Unicode code-point order. Each
group array is accompanied by a count key named after the group plus
`Count` (`leftOnlyCount`, `rightOnlyCount`, `sameCount`, `diffCount`). When
neither snapshot holds any resource, the `resources` array and all four
groups are empty and all counts are `0`.
...
```json
{"diff":[{"fields":["occupied","remaining"],"resourceId":"pool"}],"diffCount":1,"leftOnly":[],"leftOnlyCount":0,"resources":[{"equal":false,"left":{"capacity":0,"occupied":0,"remaining":0},"resourceId":"only-r","right":{"capacity":7,"occupied":4,"remaining":3}},{"equal":false,"left":{"capacity":10,"occupied":3,"remaining":7},"resourceId":"pool","right":{"capacity":10,"occupied":5,"remaining":5}},{"equal":true,"left":{"capacity":5,"occupied":1,"remaining":4},"resourceId":"shared","right":{"capacity":5,"occupied":1,"remaining":4}}],"rightOnly":["only-r"],"rightOnlyCount":1,"same":["shared"],"sameCount":1}
```

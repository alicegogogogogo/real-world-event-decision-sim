...
### `GET /snapshots/{snapshotId}/events/region/aggregate?organizationId=...&region=...&type=...&windowSize=...`

A read-only region window aggregate over one snapshot's captured events for
the caller's organization — the single-snapshot counterpart of
`GET /events/region/aggregate`, lowering the exact region and window
contract of the main region aggregate onto the events the snapshot captured
...
Query parameters follow the exact same rules as
...
`from` and `to` must be omitted together or supplied together, and must
satisfy `from <= to`. Only the snapshot's captured events matching the
caller's organization, the region, and `type` are counted; region
attribution follows the main rule (only a non-empty string payload `region`
attributes an event, matched verbatim with no normalization), an unknown
region simply matches zero events and is never implicitly created, and
other organizations' data and events committed after capture never enter
the result.

Windows start at `0` and each covers `[start, end)` with
`end = start + windowSize`; an event belongs to a window when
`start <= occurredAt < end` — boundaries identical to the main event and
region aggregates. The `200` response echoes the organization, snapshot,
region, type, window width and the (possibly null) time range:
...
- With `from`/`to`, only captured events with `from <= occurredAt <= to`
  ...
  returned, including empty windows with `count: 0`.
- `windows` is sorted by `start` ascending; results are deterministic
  regardless of event insertion order.

The response is compact JSON with keys sorted by code point, integer values
kept as integers, and one trailing newline:

```json
{"from":null,"organizationId":"org-1","region":"north","snapshotId":"snap-1","to":null,"type":"incident.created","windowSize":60,"windows":[{"count":2,"end":60,"start":0}]}
```

```bash
curl 'http://127.0.0.1:8000/snapshots/snap-1/events/region/aggregate?organizationId=org-1&region=north&type=incident.created&windowSize=60&from=0&to=180' \
  -H 'Authorization: Bearer tok-1'
```
...
A read-only, step-by-step recomputation of the window counts and peak
decision over one snapshot's captured events for the caller's organization —
the single-snapshot counterpart of `GET /events/replay/decisions`, lowering
the exact window and peak contract of the main replay-decision query onto
the events the snapshot captured at its creation time. Snapshots are
immutable, so nothing in the snapshot, in any branch, or in the main service
is read for mutation or written: the main-service event ledger, reservation
inventory, and alert state are untouched, failed requests leave no trace,
and identical requests return byte-for-byte identical JSON. Both `read` and
...
`from` and `to` must be omitted together or supplied together, and must
satisfy `from <= to`. Only the snapshot's captured events matching both the
caller's organization and `type` are replayed; other organizations' data and
events committed after the capture never enter the result. The matching
captured events are ordered by `occurredAt` ascending and then `eventId` in
Unicode code-point order, and accumulated into the replay in that order.
Each replay step reports the event's `eventId` and `occurredAt`, the
window-count rows for every event accumulated up to and including that one,
and the peak decision at that point. Windows start at `0` and each covers
`[start, start + windowSize)` — boundaries identical to the main aggregate
and replay-decision queries:
...
At each step the window rows are exactly what
`GET /snapshots/{snapshotId}/events/aggregate` returns on the accumulated
prefix and the peak fields are exactly what
`POST /snapshots/{snapshotId}/decisions/evaluate` returns on it. The `200`
response echoes the organization, snapshot, type, window width, threshold,
and the (possibly null) range, and carries the ordered `steps` array. It is
compact JSON with keys sorted by code point, integer values kept as
integers, and one trailing newline:
...
- `401 Unauthorized` (`{"error": "unauthorized"}`) — the Bearer credential
  ...
  content.
- `422 Unprocessable Entity` (`validation_error`) — a parameter is missing,
  duplicated, blank, or invalid: a non-positive-integer
...
caller's organization — the single-snapshot counterpart of the two-snapshot
window decision (`POST /snapshots/compare`), lowering the exact window and
peak contract of `POST /decisions/evaluate` onto the events the snapshot
captured at its creation time. Snapshots are immutable, so nothing in the
snapshot, in any branch, or in the main service is read for mutation or
written: the main-service event ledger, reservation inventory, and alert
state are untouched, failed requests leave no trace, and identical requests
return byte-for-byte identical JSON. Both `read` and `write` credentials
may call it. Requires `Content-Type: application/json`. The body must be a
...
Windows start at `0` and cover `[start, start + windowSize)` — boundaries
identical to the main decision and the snapshot window comparison. Without
`from`/`to`, only windows actually covered by matching captured events are
considered; with a range, captured events are filtered by the closed
interval `from <= occurredAt <= to` and every window intersecting that
interval is considered, including empty ones counted as zero. The `200`
response echoes the organization, snapshot, type, window width and the
(possibly null) time range, and adds the peak result:
...
- `peakCount` is the largest window count and `peakStart` is that window's
  start; ties resolve to the earliest start, so results are reproducible
  regardless of event submission order.
- With no matching events, `peakCount` is `0`, `peakStart` is `null`, and
  `action` is `"observe"`.
- `action` is `"escalate"` when `peakCount >= threshold`, otherwise
  `"observe"`.
- Because the peak shares the window comparison's contract, comparing a
  snapshot with itself via `POST /snapshots/compare` reports this exact
  `peakStart`, `peakCount`, and `action` on both the `left` and `right`
  decision sides, item by item.
...
- `400 Bad Request` — body is not syntactically valid JSON.
- `422 Unprocessable Entity` (`validation_error`) — a non-object body, a
  missing or extra field, a blank or non-string `organizationId`/`type`,
  booleans/floats where integers are required, non-positive
  `windowSize`/`threshold`, unpaired or non-integer `from`/`to`, or
  `from > to`. No snapshot is created and nothing is written.
- `403 Forbidden` (`{"error": "forbidden"}`) — the body's organization
  ...
  organization.
- `404 Not Found` (`{"error": "snapshot_not_found"}`) — the snapshot name
  has never existed; an evaluation never implicitly creates a snapshot.
...
A read-only, resource-balance comparison of two existing branches. Where
`POST /branches/compare/reservations` aligns reservations by
`reservationId`, this entry point aligns the two branches' resources by
`resourceId` and compares each resource's three balances — `capacity`,
`occupied`, and `remaining` — as they currently stand in the branches.
Nothing in either branch, in any other branch, or in the main service is
read for mutation or written, alert state is untouched, and identical
submissions return byte-for-byte identical JSON. Both `read` and `write`
credentials may call it. Requires `Content-Type: application/json` and a
Bearer credential bound to the request's `organizationId`. The body must be
a JSON object containing exactly these fields:
...
Using the same branch name for `left` and `right` is legal; the two sides
are then the same branch and every row's `equal` marker is true.
...
When neither branch holds any resource for the organization, the `resources`
array and all four groups are empty and all four counts are `0`.
...
```bash
curl -X POST http://127.0.0.1:8000/branches/compare/resources \
  -H 'Authorization: Bearer tok-1' \
  -H 'Content-Type: application/json' \
  -d '{"organizationId":"org-1","left":"br-a","right":"br-b"}'
```

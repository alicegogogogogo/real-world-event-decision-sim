...
`POST /branches/compare/reservations` aligns two branches' reservations by
`reservationId`, this entry point aligns the two branches' resources by
`resourceId` and compares the two sides' three resource balances —
`capacity`, `occupied`, and `remaining`. Nothing in either branch, in any
other branch, or in the main service is read for mutation or written —
branch inventories and alert state are untouched, and identical submissions
return byte-for-byte identical JSON. Both `read` and `write` credentials may
call it. Requires `Content-Type: application/json` and a Bearer credential
bound to the request's `organizationId`. The body must be a JSON object
containing exactly these fields:
...
Using the same branch name for `left` and `right` is legal; the two sides
are then the same inventory and every row's `equal` marker is true.

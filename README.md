...
contract. No snapshot content, branch, main-service ledger, reservation
inventory, or alert state is read for mutation or written, and identical
...
carries one row per resource the organization had reserved at capture time,
...
resource at capture time), `occupied` (the sum of every captured
reservation the organization held against the resource — multiple
reservations all count), and `remaining` (`capacity - occupied`). A
snapshot holds only its owning organization's records, so other
organizations' reservations never contribute, and a snapshot with no
resources returns an empty `resources` array. Because the balances share
the comparison's contract, comparing a snapshot with itself via

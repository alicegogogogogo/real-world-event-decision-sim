...
`POST /branches/compare/reservations` aligns two branches' reservations,
this entry point aligns the reservations each snapshot captured at creation
time (only the creator organization's records) by `reservationId` and
reports the deterministic difference summary. Nothing in either snapshot,
in any branch, or in the main service is read for mutation or written,
...
are then equal by construction and every captured reservation lands in
`same`.
...
group array is accompanied by a count key named after the group plus
`Count` (`leftOnlyCount`, `rightOnlyCount`, `sameCount`, `diffCount`). When
neither snapshot holds any reservation, all four groups are empty arrays
and all four counts are `0`.

...
- only the snapshot's captured events of the caller's organization, the
  requested ``region`` (a non-empty string payload ``region`` matched
  verbatim), and the requested ``type`` are counted; other organizations'
  data, events committed after capture, and events without a non-empty
  string payload ``region`` never enter the result;
...
class SnapshotRegionAggregateTest(unittest.TestCase):
    ...
    def aggregate(
        ...
        *,
        ...
        return self.call(
            f"/snapshots/{snapshot}/events/region/aggregate?{query}",
            token=token,
        )
    ...
    def test_aggregates_only_matching_region_events_into_windows(self) -> None:
        ...
        self.add_event("evt-5", occurred_at=10, payload={"region": "north"},
    ...
    def test_region_matches_verbatim_with_no_normalization(self) -> None:
    ...
    def test_events_without_region_never_match(self) -> None:
        self.add_event("evt-1", occurred_at=10, payload={})
        self.add_event("evt-2", occurred_at=10, payload={"region": ""})
        self.add_event("evt-3", occurred_at=10, payload={"region": 7})
        self.add_event("evt-4", occurred_at=10, payload={"region": None})
        self.add_event("evt-5", occurred_at=10, payload={"region": ["north"]})
        self.capture()
    ...
    def test_unknown_region_is_a_zero_match_and_never_created(self) -> None:
        self.add_event("evt-1", occurred_at=10, payload={"region": "north"})
        self.capture()

        status, body = self.aggregate(self.query(region="nowhere"))
        self.assertEqual(status, 200)
        self.assertEqual(body["region"], "nowhere")
        self.assertEqual(body["windows"], [])
        # The unknown region query leaves no trace behind.
        status, body = self.aggregate(self.query())
        self.assertEqual(status, 200)
        self.assertEqual(
            body["windows"], [{"start": 0, "end": 60, "count": 1}]
    ...
    def test_query_shape_errors_are_422(self) -> None:
        ...
        bad_queries = [
            ...
            "organizationId=org-1&region=north&type=incident.created",
            "organizationId=org-1&region=north"
            "&type=incident.created&windowSize=",
            "organizationId=org-1&region=north"
            "&type=incident.created&windowSize=%20",
            "organizationId=org-1&region=north"
            "&type=incident.created&windowSize=0",
            "organizationId=org-1&region=north"

...
captured reservations; this locks down the new read-only entry point that
lists one snapshot's captured reservations for the caller's organization:

- the ``reservations`` array carries one row per reservation the
  organization held when the snapshot was captured, sorted by resourceId
  then reservationId in Unicode code-point order, each row reporting the
  five fields ``organizationId``, ``reservationId``, ``resourceId``,
  ``quantity`` and ``capacity`` — the same field names the reservation
  comparison uses;
...
- the rows share the comparison's exact contract: a snapshot compared
  with itself via POST /snapshots/compare/reservations reports every
  listed reservation in its ``same`` group with zero diffs, and other
  organizations' reservations never contribute;
...
class SnapshotReservationListTest(unittest.TestCase):
    ...
    def test_rows_sort_by_resource_then_reservation_in_code_point_order(
        ...
        self.add_reservation("book-b", resource_id="res-a", quantity=1, capacity=9)
        self.add_reservation("book-A", resource_id="res-a", quantity=2, capacity=9)
        self.add_reservation("book-a", resource_id="res-a", quantity=3, capacity=9)
        self.add_reservation("book-1", resource_id="res-1", quantity=1, capacity=4)
        self.add_reservation("book-z", resource_id="res-B", quantity=1, capacity=4)
        self.capture()
    ...
    def test_listing_reflects_capture_time_only(self) -> None:
        ...
        self.add_reservation("res-3", resource_id="res-late", quantity=1, capacity=9)
    ...
    def test_repeated_requests_are_byte_identical(self) -> None:
        ...
        raws = [
            self.raw("/snapshots/s1/reservations?organizationId=org-1", token="w1")[1]
            for _ in range(3)
        ]
    ...
    def test_self_comparison_agrees_row_by_row(self) -> None:
        ...
        self.assertEqual(
            sorted(row["reservationId"] for row in listing["reservations"]),
            comparison["same"],
        )
    ...
    def test_query_shape_errors_are_422(self) -> None:
        ...
        self.assertEqual(self.call("/snapshots")[1]["snapshots"][0]["reservations"], 0)

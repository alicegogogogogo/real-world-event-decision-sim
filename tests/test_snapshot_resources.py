...
The snapshot resource comparisons already aligned two snapshots'
balances; this locks down the new read-only entry point that lists one
snapshot's captured resource balances for the caller's organization:

- the ``resources`` array carries one row per resource the organization
  reserved when the snapshot was captured, code-point sorted, each with
  the ``capacity``, ``occupied`` and ``remaining`` balances; ``occupied``
  sums every captured reservation the organization holds against the
  resource and ``remaining`` is ``capacity - occupied``;
...
- the balances share the comparison's exact contract: a snapshot compared
  with itself via POST /snapshots/compare/resources reports every row
...
- snapshots are immutable: later main-service writes never change the
  ...
  alert state; restarting clears snapshots and balances.
"""
...
class SnapshotResourceListTest(unittest.TestCase):
    ...
    def test_other_organizations_never_contribute(self) -> None:
        ...
        # ORG2's own snapshot listing sees only ORG2 captured balances.
    ...
    def test_foreign_snapshot_is_403_not_404(self) -> None:
        ...
        status, body = self.list_resources("s2")
        self.assertEqual(status, 403)
        self.assertEqual(body["error"], "forbidden")

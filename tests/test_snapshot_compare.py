...
The snapshot-level event and reservation comparisons already existed; this
locks down the new read-only entry point that compares two snapshots' window
counts and peak decisions, recomputed from the events each snapshot
captured:

- each side is recomputed from that snapshot's captured events with the
...
class SnapshotCompareTest(unittest.TestCase):
    ...
    def test_compares_union_of_hit_windows_aligned_by_start(self) -> None:
        # snap-left captures two events in window [0,60); snap-right later
        # captures those plus one in [60,120) and two in [120,180).
        self.add_main_event("evt-a", 5)
        self.add_main_event("evt-b", 10)
        self.capture("snap-left")
        self.add_main_event("evt-c", 65)
        self.add_main_event("evt-d", 120)
        self.add_main_event("evt-e", 125)
    ...
    def test_range_keeps_empty_windows_and_filters_events(self) -> None:
        ...
        self.add_main_event("evt-b", 10)
        self.capture("snap-left")
        self.add_main_event("evt-c", 65)  # outside [0, 60]
        self.add_main_event("evt-d", 120)  # outside [0, 60]
        self.capture("snap-right")
        ...
        # Peaks are computed over the same filtered events: both sides peak
        # at 0 with 2.
    ...
    def test_range_includes_trailing_window_hit_by_neither_side(self) -> None:
        ...
        # snap-right was captured after evt-b joined the ledger, so it holds
        # both events in the first window while snap-left holds only evt-a.
        self.assertEqual(
            body["windows"][0],
    ...
    def test_threshold_decides_escalate_vs_observe_per_side(self) -> None:
        # Both snapshots peak at count 2 in window [0,60); the threshold
        # alone decides whether each side escalates or observes.
        self.add_main_event("evt-a", 5)
        self.add_main_event("evt-b", 10)
        self.capture("snap-left")
        self.add_main_event("evt-c", 120)
        self.capture("snap-right")
        ...
        # With threshold 3 neither side escalates.
    ...
    def test_peak_tie_resolves_to_earliest_window_start(self) -> None:
        self.add_main_event("evt-a", 65)
        self.add_main_event("evt-b", 125)
        self.capture("snap-left")
        self.capture("snap-right")
        status, body = self.compare()
        self.assertEqual(status, 200)
    ...
    def test_later_main_events_never_reach_an_earlier_snapshot(self) -> None:
        # The comparison is recomputed from the captured events only: events
        # committed after a capture are invisible to that snapshot forever.
        self.add_main_event("evt-a", 5)
        self.capture("snap-left")
        self.add_main_event("evt-b", 10)
        self.add_main_event("evt-c", 15)
        self.capture("snap-right")
        ...
        self.assertEqual(
            body["windows"],
            [{"start": 0, "leftCount": 1, "rightCount": 3, "equal": False}],
        )
        self.assertEqual(body["decision"]["left"]["peakCount"], 1)
    ...
    def test_multi_organization_snapshots_compare_within_their_owner_only(
        ...
        self.add_main_event("evt-b", 65)
        self.capture("snap-right")
        # ORG2 commits its own events and captures its own snapshots.
        self.add_main_event("evt-f", 120, token="w2")
        self.capture("foreign-a", token="w2")
        self.add_main_event("evt-g", 125, token="w2")
        ...
        self.assertEqual(
            body["windows"],
            [{"start": 120, "leftCount": 1, "rightCount": 2, "equal": False}],
        )
        self.assertEqual(body["decision"]["left"]["action"], "escalate")
        self.assertEqual(body["decision"]["right"]["action"], "escalate")
    ...
    def test_compare_is_not_available_on_get(self) -> None:
        self.capture("snap-left")
        self.capture("snap-right")
        # The comparison exists only as POST; GET falls through to the
        # standard JSON 404.

...
HTTP: a committed eventId is immutable in the main ledger, so later
snapshots only gain identifiers. The ``diff`` classification (and payload
key-order invariance for genuinely different text) is therefore also
exercised directly against captured :class:`Snapshot` objects, where the
same eventId may legitimately carry different fields.
"""
...
class SnapshotEventCompareTest(unittest.TestCase):
    ...
    def add_main_event(
        ...
        **kwargs: Any,
        ...
        status, _ = self.call(
            ...
            token=token,
            payload=event_body(
                event_id, occurred_at, organization_id=organization_id, **kwargs
            ),
        )
    ...
    def test_identifiers_sort_in_code_point_order(self) -> None:
        for event_id in ("evt-b", "evt-A", "evt-a", "evt-1"):
            self.add_main_event(event_id, 10)
        self.capture("snap-left")
        self.capture("snap-right")
        status, body = self.compare()
        self.assertEqual(status, 200)
    ...
    def test_diff_classification_names_mismatched_fields(self) -> None:
        # Main-service records are immutable per eventId, so build the
        # captured snapshots directly: the same eventId carries different
        # occurredAt/payload/type on the two sides.
        left = Snapshot(
            "snap-left",
            ORG1,
            {
                ...
                "evt-op": event_record(
                    "evt-op", 20, payload={"severity": "low"}
                ),
                "evt-typed": event_record("evt-typed", 30),
        ...
        right = Snapshot(
            ...
            ORG1,
            {
                "evt-same": event_record("evt-same", 10),
                "evt-op": event_record(
                    "evt-op", 25, payload={"severity": "high"}
                ),
                "evt-typed": event_record(
                    "evt-typed", 30, event_type="incident.updated"
                ),
        ...
        self.assertEqual(
            result["diff"],
            [
                {"eventId": "evt-op", "fields": ["occurredAt", "payload"]},
    ...
    def test_organization_field_participates_in_diff(self) -> None:
        left = Snapshot(
            "snap-left",
            ORG1,
            {"evt-1": event_record("evt-1", 10, organization_id=ORG1)},
            {},
            {},
        )
        right = Snapshot(
            "snap-right",
            ORG1,
            {"evt-1": event_record("evt-1", 10, organization_id="org-other")},
            {},
            {},
        )
        result = compare_snapshot_events(
        ...
        self.assertEqual(
            result["diff"],
            [{"eventId": "evt-1", "fields": ["organizationId"]}],
        )
    ...
    def test_compare_is_read_only(self) -> None:
        ...
        self.assertEqual(status, 200)
        self.assertEqual(
            sorted(event["eventId"] for event in events["events"]),
            ["evt-a", "evt-b"],
        )
    ...
    def test_malformed_authorization_header_is_401(self) -> None:
        self.capture("snap-left")
        self.capture("snap-right")
        raw_body = json.dumps(self.compare_payload()).encode()
        # A non-Bearer scheme is just as unauthenticated as a missing header.
        request = Request(
            f"{self.base_url}/snapshots/compare/events",
            data=raw_body,
            headers={
    ...
    def test_validation_errors_are_422_and_write_nothing(self) -> None:
        ...
        bad_payloads: list[Any] = [
            ...
            {**valid, "extra": 1},
            {**valid, "type": EVENT_TYPE},  # branch-window fields not allowed
            {**valid, "left": ""},
            {**valid, "right": "   "},
